...
namespace CqrsWebApi.Infrastructure.Repositories;
...
public class InvoiceRepository(AppDbContext dbContext) : IInvoiceRepository
{
	...
	public async Task<Invoice?> UpdateAsync(Invoice invoice, CancellationToken cancellationToken)
	{
		var existingInvoice = await dbContext.Invoices
								 .Include(x => x.InvoiceItems)
								 .SingleOrDefaultAsync(x => x.Id == invoice.Id, cancellationToken);
		if (existingInvoice == null) throw new InvalidOperationException($"Invoice with id {invoice.Id} not found.");
	...
	{
		var invoice = await dbContext.Invoices.FindAsync(new object?[] { id }, cancellationToken);
		if (invoice == null) throw new InvalidOperationException($"Invoice with id {id} not found.");
