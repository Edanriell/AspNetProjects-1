...
namespace CqrsWebApi.Core.Services.Implementations;
...
public class InvoiceService(IInvoiceRepository invoiceRepository, IMapper mapper) : IInvoiceService
{
	...
	public async Task<List<InvoiceWithoutItemsDto>> GetPagedListAsync(int pageIndex, int pageSize,
	...
	public async Task<InvoiceDto> AddAsync(CreateOrUpdateInvoiceDto invoice,
										   CancellationToken cancellationToken = default)
	{
	...
	public async Task<InvoiceDto?> UpdateAsync(Guid id, CreateOrUpdateInvoiceDto invoice,
											   CancellationToken cancellationToken = default)
	{
		var invoiceEntity = mapper.Map<Invoice>(invoice);
		invoiceEntity.Id = id;
