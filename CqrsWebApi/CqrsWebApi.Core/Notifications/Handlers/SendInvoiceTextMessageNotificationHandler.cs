...
namespace CqrsWebApi.Core.Notifications.Handlers;

public class SendInvoiceTextMessageNotificationHandler(IInvoiceService invoiceService)
	: INotificationHandler<SendInvoiceNotification>
{
	public async Task Handle(SendInvoiceNotification notification, CancellationToken cancellationToken)
	{
		...
		if (invoice is null || string.IsNullOrWhiteSpace(invoice.ContactPhone)) return;
		// Send text message notification
