...
public class LoggingController(ILogger<LoggingController> logger) : ControllerBase
{
    ...
    public ActionResult ExceptionSample()
    {
        try
        {
            ...
            if (randomNumber == 3) throw new Exception("This is a generic exception");
        }
        catch (Exception ex)
        {
            logger.LogError(ex,
                "This is an exception logging message. Datetime: {exceptionDateTime}. Exception message: {exceptionMessage}",
