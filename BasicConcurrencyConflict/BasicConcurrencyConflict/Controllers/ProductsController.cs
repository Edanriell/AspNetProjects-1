...
namespace BasicConcurrencyConflict.Controllers;
...
public class ProductsController(SampleDbContext context) : ControllerBase
{
    ...
    {
        if (context.Products == null) return NotFound();
    ...
    {
        if (context.Products == null) return Problem("Entity set 'SampleDbContext.Products' is null.");
    ...
    public async Task<ActionResult<Product>> SellProduct(int id, int quantity, int delay = 0)
    {
        ...
        var product = await context.Products.FindAsync(id);
        if (product == null) return NotFound();
        if (product.Inventory < quantity) return Problem("Not enough inventory.");
        ...
        // await context.SaveChangesAsync();
        // return product;
        // If we use application-managed concurrency token
        try
        {
            await context.SaveChangesAsync();
        }
