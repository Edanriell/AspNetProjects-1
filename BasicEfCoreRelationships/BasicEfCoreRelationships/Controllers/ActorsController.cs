...
public class ActorsController(SampleDbContext context) : ControllerBase
{
    ...
    {
        if (context.Actors == null) return NotFound();
    ...
    {
        if (context.Actors == null) return Problem("Entity set 'SampleDbContext.Actors'  is null.");
    ...
    {
        if (context.Actors == null) return NotFound("Actors is null.");

        var actor = await context.Actors.Include(x => x.Movies).SingleOrDefaultAsync(x => x.Id == id);
        if (actor == null) return NotFound($"Actor with id {id} not found.");
