...
namespace BasicRouting.Services;
...
public class PostsService : IPostService
{
    ...
    {
        var post = AllPosts.FirstOrDefault(x => x.Id == id);
        if (post != null) AllPosts.Remove(post);
    ...
    {
        return Task.FromResult(AllPosts.Where(x => x.Title.Contains(keyword, StringComparison.OrdinalIgnoreCase))
            .ToList());
    }
}