...
builder.Services.AddHttpClient("PollyWebApi.Client",
	client => { client.BaseAddress = new Uri("http://localhost:5062"); });

// Add a timeout policy
builder.Services.AddResiliencePipeline("timeout-5s-pipeline",
	configure => { configure.AddTimeout(TimeSpan.FromSeconds(5)); });
