...
namespace InvoiceAppV2.Repositories;
...
public class InvoiceRepository(InvoiceDbContext dbContext) : IInvoiceRepository
{
    ...
    public async Task<IEnumerable<Invoice>> GetInvoicesByContactIdAsync(Guid contactId, int page = 1, int pageSize = 10,
        InvoiceStatus? status = null)
    {
        return await dbContext.Invoices
            .Include(x => x.Contact)
    ...
    public async Task<Invoice> CreateInvoiceAsync(Invoice invoice)
    {
        ...
        if (contact == null) throw new ArgumentException("Contact not found", nameof(invoice.ContactId));
